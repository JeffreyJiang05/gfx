...
template<typename T>
constexpr vec<2, T>::vec() : _data{} {}

// initialize data explicitly
template<typename T>
constexpr vec<2, T>::vec(value_type x, value_type y) : _data{ x, y } {}

// copy constructible from castible type
template<typename T>
template<typename U>
constexpr vec<2, T>::vec(const vec<2,U>& cpy) : _data{ static_cast<T>(cpy[0]), static_cast<T>(cpy[1]) } {}

// initialize data of convertible type
template<typename T>
template<typename T0, typename T1>
constexpr vec<2, T>::vec(T0 x, T1 y) : _data{ static_cast<T>(x), static_cast<T>(y) } {}
...
template<typename T>
constexpr auto vec<2, T>::operator[](unsigned idx) const -> const value_type&
{
    return _data[idx];
}
...
template<typename T>
constexpr auto vec<2, T>::at(unsigned idx) const -> const value_type&
{
    if (idx >= 2) throw std::out_of_range{ "Index " + std::to_string(idx) + " out of range for vec2." };
    return _data[idx];
}
...
template<typename T>
constexpr auto vec<2,T>::x() const -> value_type
{
    return _data[0];
}

template<typename T>
constexpr auto vec<2,T>::y() const -> value_type
{
    return _data[1];
}
